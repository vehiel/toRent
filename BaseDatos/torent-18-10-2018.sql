...
insert  into `tr08nhr`(`idn_08in`,`nom_08vc`) values 
(1,'Taladro'),
(2,'Patin'),
(3,'Sander'),
(4,'Batidora'),
...
insert  into `tr10her`(`chr_10in`,`idn_08in`,`cgm_09in`,`vol_10in`,`des_10vc`,`vut_10in`,`gar_10in`,`tip_10in`,`est_10in`,`alq_10in`,`ser_10vc`) values 
(1,1,1,1,'amarrillo con negro',5,12,1,1,0,'123qwerty');

/*Table structure for table `tr11alq` */

DROP TABLE IF EXISTS `tr11alq`;

CREATE TABLE `tr11alq` (
  `nsa_11in` int(10) NOT NULL AUTO_INCREMENT COMMENT 'Número alquiler',
  ...
  `ede_11in` int(5) DEFAULT NULL COMMENT 'Estado devolución',
  `est_11in` int(5) NOT NULL COMMENT 'Estado',
  `obs_11vc` varchar(100) COLLATE utf8_unicode_ci NOT NULL COMMENT 'Observaciones',
  PRIMARY KEY (`nsa_11in`),
  KEY `fk_usuario_alquiler` (`nus_02in`),
  KEY `fk_herramienta_alquiler` (`chr_10in`),
  KEY `fk_cliente_alquiler` (`ncl_06in`),
  CONSTRAINT `fk_cliente_alquiler` FOREIGN KEY (`ncl_06in`) REFERENCES `tr06cli` (`ncl_06in`),
  CONSTRAINT `fk_herramienta_alquiler` FOREIGN KEY (`chr_10in`) REFERENCES `tr10her` (`chr_10in`),
  CONSTRAINT `fk_usuario_alquiler` FOREIGN KEY (`nus_02in`) REFERENCES `tr02usu` (`nus_02in`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;

/*Data for the table `tr11alq` */
